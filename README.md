...
This copy has a TMDB API key **embedded directly in `script.js`** (see `EMBEDDED_API_KEY` near the top of the file), so it works immediately with no setup. Because it's client-side code, that key is visible to anyone who views the page source or inspects network requests — treat it as public, not secret. Swap in your own key by editing that constant, or paste a different one into the Save Key box in the app (which takes priority and is stored in your browser's `localStorage`).

## 🔑 Getting a free TMDB API key (optional, if you want your own)
